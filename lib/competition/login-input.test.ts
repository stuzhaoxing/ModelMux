...
describe("unified login input", () => {
  ...
  it("still accepts an omitted or contestant role", () => {
    ...
    const parsed = loginInputSchema.parse({
      ...
      password: "b",
      role: "contestant",
      next: "/contestant/questions",
    });
    expect(parsed.username).toBe("Contestant-1");
    expect(parsed.role).toBe("contestant");
    expect(parsed.next).toBe("/contestant/questions");
  });
  ...
  it("rejects empty, oversized or unknown-role input", () => {
    ...
    expect(loginInputSchema.safeParse({ username: "a", password: "b", role: "judge" }).success).toBe(false);
  });
});
