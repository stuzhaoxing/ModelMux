...
- 无需密码的比赛大屏实时显示真实比赛状态、倒计时、全部选手答题进度和本场 Token 消耗趋势；赛前可由管理员发布巨型公共公告
- 评委端可一键导出全部已发布/已关闭题目的答卷 ZIP；ZIP 按选手分目录，每道题同时生成 Word 和 PDF，未作答也会保留明确标注
- MySQL 持久化考核数据，题目和答卷的图片及任意附件流式保存在服务器本地数据目录
- 管理员可编辑选手姓名、登录账号和密码；密码留空保留原密码，修改登录凭证后旧会话失效，选手需重新登录，答卷和 API Key 保留
...
选手 API Key 随账号保存在 MySQL；运维客户端 Key、供应商 Key 和路由仍使用环境变量管理。最近请求元数据保存在进程内存，本场比赛分钟级 Token 总量保存在 MySQL，但不记录个人用量或设置账号额度。运行模式由 MySQL 中的比赛阶段和开始记录决定；考核账号、题目、答卷和时间记录也持久化到 MySQL。旧的模式与停服状态文件不再读取。
...
| 比赛投屏 | `http://localhost:1444/screen` | 无需登录，只读展示真实比赛数据 |
| 管理员控制台 | `http://localhost:1444/admin` | 独立管理密码，Cookie 为 `modelmux_admin_session` |

评委和选手共用一个会话 Cookie，因此同一个浏览器同一时刻只能是其中一个身份，后登录的会顶掉先登录的；要同时开两端请用两个浏览器或无痕窗口。管理员 Cookie 与它们相互独立，可以同时保持登录；比赛大屏 `/screen` 及其只读快照接口无需密码，直接展示真实比赛数据。同一个账号名同时存在于评委和选手时，登录页会让本人再选一次角色。后台页面和 `/api/admin/*` 均强制验证管理员会话；模型 API 另用客户端 API Key，不复用任何网页登录凭据。

评委在 Dashboard 填写比赛时长后开始比赛；开始状态下选手同时看到整套题目并可正常保存、提交，`/screen` 同步显示倒计时。评委可随时停止比赛，停止或自然到时后选手端隐藏全部题目，只显示“比赛已结束”；再次开始会重新计时并保留历史答案。首次开始正式比赛前自动进入不限时的测试阶段，已保存的测试题自动对选手开放，可保存和提交测试答案；无需手动开放测试或设置测试时长。比赛未运行时，评委可在 `/judge/answers` 删除题目，删除操作会同时永久删除该题已有答卷；比赛进行中禁止删除。默认时长由 `MODELMUX_COMPETITION_DURATION_MINUTES` 控制，必须为正整数，默认 90 分钟。

评委在 Dashboard 点击“导出全部答卷”即可下载归档 ZIP。导出会把选手回答中的图片嵌入 Word/PDF，普通附件仍以文件名、大小和站内下载地址写入文档，不会把任意大的普通附件复制进归档；附件仍保存在 `MODELMUX_DATA_DIR/uploads`，可从系统中下载。比赛未进行时，管理员可在考务工作台填写并开启“赛前大屏公告”，用于集中展示 API URL 等公共提醒；公告只在比赛尚未开始时覆盖大屏，开始比赛后自动撤下。大屏每 3 秒刷新数据，选手超过单屏容量时自动调整布局。
...
| --- | --- | --- | --- |
| `deepseek-v4-flash` | DeepSeek 官方 | 1M | 单一原厂路由 |
| `deepseek-v4-pro` | DeepSeek 官方 | 1M | 单一原厂路由 |
| `qwen3.7-flash` | 阿里云百炼 | 1M | 单一原厂路由 |
| `qwen3.7-plus` | 阿里云百炼 | 1M | 单一原厂路由 |
| `qwen3.7-max` | 阿里云百炼 | 1M | 映射到 `qwen3.7-max-2026-06-08` 多模态快照 |
...
公网和本地实例分别保存环境变量、密钥和运行日志。模型 API 在两种环境中均保持开启。赛中终端不设置默认网关和 DNS，只能访问 Mac mini 上的江苏省监测技能竞赛在线答题系统。

## 运行模式

运行模式由考务总览的比赛开始动作自动决定。网关总览和系统设置页均已移除；旧地址 `/admin`、`/admin/settings` 自动跳转到 `/admin/competition`。

| 模式 | 大屏状态 | 模型 API | 典型用途 |
| --- | --- | --- | --- |
| 测试模式 | 测试演练中 | 鉴权、白名单路由后转发 | 赛前公网联调、设备演练 |
| 比赛模式 | 比赛进行中或已结束 | 鉴权、白名单路由后转发 | 正式比赛 |

正式比赛开始前为测试模式，测试不限时，测试题保存后自动开放。点击“开始正式比赛”并成功启动后，自动进入比赛模式；停止比赛或自然到时后仍保留比赛模式和结束状态。模式从 MySQL 比赛记录读取，刷新和重启后保持一致；未配置比赛数据库的纯网关实例使用测试模式。

两种模式都不设置账号调用额度、RPM 或请求体大小上限。模式不干预请求参数和上游响应；模型白名单、供应商路由与 API Key 鉴权仍然生效，模型 API 始终开启。
