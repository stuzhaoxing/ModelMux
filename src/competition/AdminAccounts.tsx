...
import type { CompetitionUser } from "@/lib/competition/types";
import { apiRequest, formatCompetitionTime } from "./api";

interface GeneratedContestant {
  role: "contestant";
  username: string;
  displayName: string;
  password: string;
...
type CopyableContestant = Pick<
  CompetitionUser,
  "username" | "password" | "displayName" | "apiKey"
>;
...
export default function AdminAccounts() {
  ...
  const loadUsers = useCallback(async () => {
    try {
      ...
      setUsers(result.users.filter((user) => user.role === "contestant"));
  ...
  async function createAccount(event: React.FormEvent) {
    ...
    try {
      await apiRequest("/api/admin/competition/users", {
        method: "POST",
        body: JSON.stringify({ role: "contestant", username, displayName, password }),
      });
      setUsername("");
      setDisplayName("");
      setPassword("");
  ...
  async function generateAccount() {
    ...
    setNotice(null);
    try {
      const result = await apiRequest<{ user: GeneratedContestant; apiBase: string }>("/api/admin/competition/users", {
        method: "POST",
        body: JSON.stringify({ role: "contestant", autoGenerate: true }),
      });
      setGeneratedCredentials(result.user);
      setApiBase(result.apiBase);
      await loadUsers();
      setNotice("选手账号已自动生成，请及时复制登录信息");
    } catch (generateError) {
  ...
  async function updateAccount(
    ...
    setNotice(null);
    try {
      await apiRequest(`/api/admin/competition/users/${user.id}`, {
        method: "PATCH",
  ...
  async function copyAccount(user: CopyableContestant) {
    ...
    const content = [
      `登录地址：${window.location.origin}/contestant/questions`,
      `账号：${user.username}`,
      `密码：${user.password}`,
    ...
    <div className="account-management">
      <section className="workspace-panel account-create-panel">
        <div className="panel-heading">
          <div><span className="field-label">CONTESTANT ACCOUNT ISSUANCE</span><h2>生成选手账号</h2></div>
          <UsersRound size={19} className="panel-icon" />
        </div>
        <form className="account-form" onSubmit={createAccount}>
          <button className="account-auto-button" disabled={pending} type="button" onClick={() => void generateAccount()}><Sparkles />自动生成选手账号</button>
          <label>登录账号<input required minLength={2} maxLength={64} pattern="[a-zA-Z0-9._-]+" value={username} onChange={(event) => setUsername(event.target.value)} placeholder="例如 contestant01" /></label>
          ...
          <button className="account-create-button" disabled={pending} type="submit">{pending ? <LoaderCircle className="spinning" /> : <Plus />}{pending ? "正在创建" : "创建选手账号"}</button>
        </form>
        {generatedCredentials && <div className="generated-credentials">
          ...
          {generatedCredentials.apiKey && <code>{generatedCredentials.apiKey}</code>}
          <button type="button" className="secondary-action" onClick={() => void copyAccount(generatedCredentials)}><Copy />复制登录信息</button>
        </div>}
        <div className="portal-links contestant-only"><a href="/contestant/questions" target="_blank">打开选手登录入口</a></div>
      </section>

      <section className="workspace-panel account-list-panel">
        <div className="panel-heading table-heading">
          <div><span className="field-label">CONTESTANTS</span><h2>选手账号列表</h2></div>
          <div className="account-heading-actions">
            <a className="account-export-button" href="/api/admin/competition/users/export?role=contestant" download><Download />导出选手 Excel</a>
